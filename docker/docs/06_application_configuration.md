...
> This is only possible after configuring Intel GVT-g on the Proxmox host -- complete
> **Section 10** before enabling these settings.
...
# 10. Proxmox â€” Intel iGPU Passthrough

**Role:** The docker-vm runs on a Proxmox host (Intel NUC NUC7i3BNH, i3-7100U). The NUC's
iGPU is not exposed to the VM by default -- it must be explicitly shared using Intel GVT-g,
a GPU virtualisation technology built into Kaby Lake and natively supported by Proxmox.

This section configures GVT-g on the Proxmox host and verifies VA-API access inside the
docker-vm, enabling the hardware transcoding settings in Plex (section 8.3) to function.
...
## 10.1 â€” Proxmox Host Configuration
...
Append these modules to `/etc/modules`:
...
## 10.2 â€” docker-vm Configuration
...
With `/dev/dri` confirmed and VA-API working, return to **section 8.3** to enable
hardware transcoding in Plex. No Compose changes are required -- the stack already
includes the correct `devices` and `group_add` configuration.

---

## 10.3 â€” Persistence Verification
